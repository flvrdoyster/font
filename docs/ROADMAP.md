...
2. **부품 에디터** — 셀을 채우는 도구. **대표 음절을 그리면 자동 추출**하는 방식이
   좋다(부품만 따로 그리면 글자 균형을 못 본다). 셀마다 자유로운 자모가 정확히
   하나라서(초중성셀이면 종성이, 종성셀이면 초성이) 거기에 단순한 자모를 넣으면
   된다. 오버레이로 ①그 셀을 쓰는 확정 음절들 ②해당 글자의
   `thin_vertical(Bold)`를 띄운다 — 후자가 원본 뼈대 근거.
   - 그릴 것 **439셀**, 검수 대상 **590셀**(`--report` 상위가 전부 `ㅟ ㅚ ㅠ ㅘ`
     같은 복합·가로모음의 받침 — 그 조합에서 받침 위치가 가장 미묘하다는 뜻).
