...
| **E: 중성도 초성별 (채택)** | **2,163** | **560** | **85.5%** | **0.91px** | **90.7%** |

### 채택안

- **벌식 E**: 초성벌 = (중성, 받침유무) / 중성벌 = (받침유무, 초성) /
  종성벌 = (중성). 부품셀 2,163개.
...
1. **부품 모델 교체** — `compose_components.py`를 벌식 E로. 부품 = `(자모, 벌)`
   셀, 폴백·다수결 최소화.
2. **부품 에디터** — 음절이 아니라 **부품 셀**을 그리는 모드가 필요(신규).
   비어 있는 560셀을 그리고, 코퍼스에서 추출된 1,603셀은 존 절단이 불안정하니
   검수 대상으로 표시. 오버레이로 ①그 셀을 쓰는 확정 음절들 ②해당 글자의
