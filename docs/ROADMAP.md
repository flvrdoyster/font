...
- **가나(히라가나·가타카나, PC-98 기준 169자)**: 손그림 착수했다가 보류 — 83/169
  히라가나 확정 상태에서, 결과가 한글보다 미묘하게 크고 덜 고딕스러워서 스타일부터
  다시 잡아야 함(재개 시 참고). Phase 3 납품은 이것과 무관하게 완료 — **미확정
  가나는 PC-98 원본 데이터로 자동 폴백**하므로 gensei-pc98에서도 정상적으로
  표시된다(아래 상세).
